...
class tm_LNK {
	...
	* @return boolean
	*/	
	#function get($id=0,$offset=0,$limit=0,$group_id=0,$search=Array(),$sortIndex="",$sortType=0,$Details=1) {
...
function createLink($lnk,$filter="",$data=Array()) {
	...
	if (check_dbid($lnk_id) && is_numeric($lnk_id)) {
		...
		if (isset($LNK[0]) && $LNK[0]['aktiv']==1) {
				...
				if ($filter=="") {
					if ($LNK[0]['url'][0]=="#") {
						$Return="\n<a href=\"".$LNK[0]['url']."\" title=\"".display($LNK[0]['descr'])."\" ".$lnk_params.">".$LNK[0]['name']."</a>\n";
					} else {
						$Return="\n<a href=\"".$tm_URL_FE."/click.php".$trackingparams."\" title=\"".display($LNK[0]['descr'])."\" target=\"_blank\" ".$lnk_params.">".$LNK[0]['name']."</a>\n";
					}
				}//""
				//only a href
				if ($filter=="ahref") {
					if ($LNK[0]['url'][0]=="#") {
						$Return="\n<a href=\"".$LNK[0]['url']." title=\"".display($LNK[0]['descr'])."\" ".$lnk_params.">\n";
					} else {
						$Return="\n<a href=\"".$tm_URL_FE."/click.php".$trackingparams."\" title=\"".display($LNK[0]['descr'])."\" target=\"_blank\" ".$lnk_params.">\n";
					}
				}//ahref
				//only url
				if ($filter=="url") {
