...
if (DEBUG) $MESSAGE.=tm_debugmessage("nl_id index is ".$nl_id_index);

if (check_dbid($FRM[0][$nl_id_index])) {
	if (DEBUG) $MESSAGE.=tm_debugmessage("nl_id for mail found ".$nl_id_index."=".$FRM[0][$nl_id_index]);
	//get newsletter
		...
		if ($NLSubscribeMail[0]['massmail']==0) {
			if ($NLSubscribeMail[0]['aktiv']==1) {
				if ($NLSubscribeMail[0]['is_template']==1) {
					if (DEBUG) $MESSAGE.=tm_debugmessage("valid newsletter found, send as mail");
					$use_nl_mail=TRUE;
				} else {
					if (DEBUG) $MESSAGE.=tm_debugmessage("not a valid newsletter, nl is not a template!");									
				}//is_template==1
			} else {
					if (DEBUG) $MESSAGE.=tm_debugmessage("not a valid newsletter, nl is not active!");
			}//aktiv==1
		} else {
			if (DEBUG) $MESSAGE.=tm_debugmessage("not a valid newsletter, nl is massmail!");
		}//massmail==0
	} else {
		if (DEBUG) $MESSAGE.=tm_debugmessage("not a valid newsletter, nl not exists!");
	}//isset [0]
} else {
	if (DEBUG) $MESSAGE.=tm_debugmessage("no valid newsletter found to send as mail, invalid id");
...
if (!$use_nl_mail) {
	if (DEBUG) $OUTPUT.=tm_debugmessage("no valid newsletter found to send as mail!");
	$OUTPUT.="ERR 3";
}
...
if ($use_nl_mail) {
	...
	//parse newsletter
	if (DEBUG) $MESSAGE.=tm_debugmessage("create mail");	
	$SubscribeMail_Subject= $NEWSLETTER->parseSubject( Array('text'=>$NLSubscribeMail[0]['subject'], 'adr'=>$ADR[0], 'date'=>$date_sub) );
	if (DEBUG) $MESSAGE.=tm_debugmessage("subject (parsed): ".$SubscribeMail_Subject);
		
	if (DEBUG) $MESSAGE.=tm_debugmessage("to: ".$ADR[0]['email']);
	...
	if (DEBUG) $MESSAGE.=tm_debugmessage("rcpt-name tmp: ".$RCPT_Name_TMP);
	$RCPT_Name = $NEWSLETTER->parseSubject( Array('text'=>$RCPT_Name_TMP, 'adr'=>$ADR[0], 'date'=>$date_sub) );
	if (DEBUG) $MESSAGE.=tm_debugmessage("rcpt-name: ".$RCPT_Name);
	//we have to replace date before parsing {DATE} --> $date_sub
	if (DEBUG) $MESSAGE.=tm_debugmessage("parse body");
	...
	if (DEBUG) $MESSAGE.=tm_debugmessage("send mail to ".$ADR[0]['email']);
	...
	if ($C[0]['notify_subscribe']==1) {
		if (DEBUG) $MESSAGE.=tm_debugmessage("send notification mail to admin! ".$C[0]['notify_mail']." , ".$HOST[0]['sender_name']);
