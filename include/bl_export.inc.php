...
if ($user_is_manager && $set=="export_blacklist") {
	...
	if (!$fp) {
			 $_MAIN_MESSAGE.= "<br>".sprintf(___("Export-Datei kann nicht geöffnet werden %s"),"<b>".$Export_Filename."</b>");
	} else {
	...
	$_MAIN_MESSAGE.= "<br>".sprintf(___("Es wurden insgesamt %s Einträge aus der Blacklist exportiert."),$exported);
