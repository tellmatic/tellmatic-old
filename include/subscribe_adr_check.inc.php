...
/*******************************************************************************/
	...
	if (DEBUG) $MESSAGE.=tm_debugmessage("precheck ok, check===true");

	#proof?!
	if ($C[0]['proof']==1) {
		if (DEBUG) $MESSAGE.=tm_debugmessage("proofing global enabled");
		if ($FRM[0]['proof']==1) {
			if (DEBUG) $MESSAGE.=tm_debugmessage("proofing for this form enabled");
			$ADDRESS->proof();
		}	else {
			if (DEBUG) $MESSAGE.=tm_debugmessage("proofing for this form disabled");		
		}
	}	
	...
	if (DEBUG) $MESSAGE.=tm_debugmessage("default group for this form: ".implode(",",$default_adr_grp));
	#$new_adr_grp=$adr_grp;
	...
	//neu= public + default
	if (DEBUG) $MESSAGE.=tm_debugmessage("user selected public group for this form: ".implode(",",$adr_grp_pub));
	$tmp_adr_grp=array_merge($adr_grp_pub,$default_adr_grp);
	if (DEBUG) $MESSAGE.=tm_debugmessage("default group + selected public group: ".implode(",",$tmp_adr_grp));
	$new_adr_grp=array_unique($tmp_adr_grp);
	if (DEBUG) $MESSAGE.=tm_debugmessage("default group + public group, unique: ".implode(",",$new_adr_grp));
	...
	if ($ac>0) {
		...
		$adr_exists=true;
		if (DEBUG) $MESSAGE.=tm_debugmessage("email already exists");
		//gruppen denen die adr bereits  angehoert
		if ($FRM[0]['overwrite_pubgroup']!=1) {
			if (DEBUG) $MESSAGE.=tm_debugmessage("updating groups, merge with old");
			$old_adr_grp = $ADDRESS->getGroupID(0,$ADR[0]['id'],0);
			if (DEBUG) $MESSAGE.=tm_debugmessage("old groups: ".implode(",",$old_adr_grp));
			$tmp_adr_grp = array_merge($old_adr_grp, $new_adr_grp);
			if (DEBUG) $MESSAGE.=tm_debugmessage("merge old and new: ".implode(",",$tmp_adr_grp));
			$new_adr_grp = array_unique($tmp_adr_grp);
			if (DEBUG) $MESSAGE.=tm_debugmessage("merge old and new, unique: ".implode(",",$new_adr_grp));
		} else {//overwrite pubgroups
			//ueberschreiben mit neuen gruppen refs:
			if (DEBUG) $MESSAGE.=tm_debugmessage("overwrite groups, save only default and new selected.");
		}//overwrite pubgroups
	} else {//adr exists
		if (DEBUG) $MESSAGE.=tm_debugmessage("email not yet exists, save new entry");
	}
	//re-index, important!
