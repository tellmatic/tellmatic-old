...
if (DEMO) $MESSAGE.="<h2><font color=\"red\">Demo</font></h2>";
...
if (isset($lang) && !empty($lang)) {
	...
	if (!DEMO) $MESSAGE_REG=TM_APPTEXT."\n".
							"Date: ".$created."\n".
	...
	if ($check) {
		...
		if ($accept==1) {
			...
			if ($set=="save" && $check) {
				...
				if ($check && $checkDB) require_once(TM_INCLUDEPATH."/install/install_examples_conf.inc.php");
				...
				if (!$check) {
					...
					require_once(TM_INCLUDEPATH."/install/install_form_show.inc.php");
...
//a copyright message
echo '<br><br>&copy;-left 2011 <a href="http://www.tellmatic.org" target="blank">Tellmatic - die Newsletter Maschine - www.tellmatic.org</a>';
echo '</div>';
//add footer
