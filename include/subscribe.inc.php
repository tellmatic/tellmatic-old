...
if (DEBUG && $called_via_url) $OUTPUT.=tm_debugmessage("called via url");
if (DEBUG && !$called_via_url) $OUTPUT.=tm_debugmessage("included");
...
if (check_dbid($frm_id)) {
	...
	if (DEBUG) $OUTPUT.=tm_debugmessage("fetch form by id");
	$FRM=$FORMULAR->getForm($frm_id);
} else {
	if (DEBUG) $OUTPUT.=tm_debugmessage("fetch default form");
	#$OUTPUT.="ERR 0";
	$FRM=$FORMULAR->getStd();
}

if (isset($FRM[0])) {
	if (DEBUG) $OUTPUT.=tm_debugmessage("form id is ".$FRM[0]['id']);
	if ($FRM[0]['aktiv']==1) {
		if (DEBUG) $OUTPUT.=tm_debugmessage("use_form=true");
		//set new form id, in the case we use standard form
		$frm_id=$FRM[0]['id'];
		$use_form=TRUE;
	} else {//aktiv
		if (DEBUG) $OUTPUT.=tm_debugmessage("form not active");
		$OUTPUT.="ERR 2";
	}//aktiv
} else {//isset FRM[0]
	if (DEBUG) $OUTPUT.=tm_debugmessage("no valid form exists");
	$OUTPUT.="ERR 1";
}

if ($doptin==1) {
	if (DEBUG) $OUTPUT.=tm_debugmessage("doptin=1");
}

//if valid form, set filenames for templates
if ($use_form) {
	if (DEBUG) $OUTPUT.=tm_debugmessage("valid form id :)");
	//formular template --> id
	$Form_Filename="Form_".$frm_id.".html";//formular
	if (DEBUG) $OUTPUT.=tm_debugmessage("use template ".$Form_Filename);
	$Form_Success_Filename="Form_".$frm_id."_success.html";//formular
	if (DEBUG) $OUTPUT.=tm_debugmessage("use template ".$Form_Success_Filename);
	//set new tpl path
	$_Tpl_FRM->setTemplatePath($tm_formpath);
}

//get default smtp host, default, as fallback
if (DEBUG) $MESSAGE.=tm_debugmessage("fetch default smtp host");
...
if ($use_form) {// && !$doptin_check
	...
	if (check_dbid($FRM[0]['host_id'])) {
		...
		if ($HOST_FRM[0]['aktiv']==1 && $HOST_FRM[0]['type']=='smtp') {
			...
			if (DEBUG) $MESSAGE.=tm_debugmessage("found valid smtp host for this form");
		} else {
			if (DEBUG) $MESSAGE.=tm_debugmessage("found no valid smtp host for this form, continue use default host");		
		}
	}	
}
if ($use_frm_smtphost) {
...
if ($use_form && $FRM[0]['use_captcha']==1) {
	if (DEBUG) $MESSAGE.=tm_debugmessage("using captcha");
}
if ($use_form && $FRM[0]['double_optin']==1) {
	if (DEBUG) $MESSAGE.=tm_debugmessage("using double-opt-in");
}
if ($use_form && $FRM[0]['force_pubgroup']==1) {
	if (DEBUG) $MESSAGE.=tm_debugmessage("using force public group");
}
if ($use_form && $FRM[0]['check_blacklist']==1) {
	if (DEBUG) $MESSAGE.=tm_debugmessage("using blacklist");
}

/********************************************************************************/
//check if double optin confirmation: user clicked on subscribe link in doptin mail, doptin==1 etc
if (DEBUG) $MESSAGE.=tm_debugmessage("checking for double-/touch-optin click: subscribe_doptin_confirm");
require_once (TM_INCLUDEPATH."/subscribe_doptin_confirm.inc.php");
/********************************************************************************/

if (!$doptin_check) {
	if (DEBUG) $MESSAGE.=tm_debugmessage("no double optin click, continue");
}
...
if ($use_form && !$doptin_check && $set=="save") {
	...
	if ($FRM[0]['double_optin']==1) {
		if (DEBUG) $MESSAGE.=tm_debugmessage("double opt in");
		$status=5;//warten auf recheck
	} else {
		if (DEBUG) $MESSAGE.=tm_debugmessage("simple single opt in");
		$status=1;//neu
	}
	$MESSAGE="";
	/********************************************************************************/
	...
	if (DEBUG) $MESSAGE.=tm_debugmessage("checking form input: subscribe_form_check");
	require_once (TM_INCLUDEPATH."/subscribe_form_check.inc.php");
	/********************************************************************************/
}
...
if ($use_form && !$doptin_check && $set=="save" && $check) {
	...
	if (DEBUG) $MESSAGE.=tm_debugmessage("check if email already exists or not, set new group refs: subscribe_adr_check");
	require_once (TM_INCLUDEPATH."/subscribe_adr_check.inc.php");
	/********************************************************************************/
	...
	if (!$adr_exists) {
		...
		if (DEBUG) $MESSAGE.=tm_debugmessage("adr record not yet exists, create new: subscribe_new");
		require_once (TM_INCLUDEPATH."/subscribe_new.inc.php");
		/********************************************************************************/
	}
	...
	if ($adr_exists) {
		/********************************************************************************/
		//update adr record
		if (DEBUG) $MESSAGE.=tm_debugmessage("adr record already exists, initiate update: subscribe_update");
		require_once (TM_INCLUDEPATH."/subscribe_update.inc.php");
		/********************************************************************************/
	}
	//next we fetch current new adr record (again)
	if (DEBUG) $MESSAGE.=tm_debugmessage("fetch adr record again:");
	$ADR=$ADDRESS->getAdr($newADRID);
	if (DEBUG) $MESSAGE.=tm_debugmessage(print_r($ADR,TRUE));
	...
	if (!$adr_exists && $FRM[0]['double_optin']==1) {
		...
		if (DEBUG) $MESSAGE.=tm_debugmessage("new record, send double optin confirmation email: subscribe_doptin_mail, nl_id_index=".$nl_id_index);
	...
	if (!$adr_exists  && $FRM[0]['double_optin']!=1) {
		...
		if (DEBUG) $MESSAGE.=tm_debugmessage("new record, no double optin, send greeting email: subscribe_mail, nl_id_index=".$nl_id_index);
		require_once (TM_INCLUDEPATH."/subscribe_mail.inc.php");
		/********************************************************************************/
	...
	if ($adr_exists) {
		...
		if (DEBUG) $MESSAGE.=tm_debugmessage("update, send update mail: subscribe_mail, nl_id_index=".$nl_id_index);
		require_once (TM_INCLUDEPATH."/subscribe_mail.inc.php");
		/********************************************************************************/
		//show update message
	...
	//add subscription
	if (!$adr_exists) {
		if (DEBUG) $MESSAGE.=tm_debugmessage("count subscription");
		$FORMULAR->addSub($frm_id,$newADRID);
	}
	//all ok, input checked and saved	
	if (DEBUG) $MESSAGE.=tm_debugmessage("check TRUE AND set == save");
...
if ($doptin_check && $doptin_success) {
	...
	$nl_id_index="nl_id_greeting";
	if (DEBUG) $MESSAGE.=tm_debugmessage("double optin succeeded. send greeting mail: subscribe_mail, nl_id_index=".$nl_id_index);
	require_once (TM_INCLUDEPATH."/subscribe_mail.inc.php");
	/********************************************************************************/
	#$MESSAGE.=display($FRM[0]['message_greeting']);
	$MESSAGE.=display($NEWSLETTER->parseSubject( Array('text'=>$FRM[0]['message_greeting'], 'adr'=>$ADR[0], 'date'=>$date_sub) ) );
...
if ($use_form && !$doptin_check && (!$check || $set!="save")) {
	if (DEBUG) $MESSAGE.=tm_debugmessage("valid Form && No double optin confirm AND (check FALSE OR set != save)");
	/********************************************************************************/
	//show form
	if (DEBUG) $MESSAGE.=tm_debugmessage("show form: subscribe_form_show");
...
if ($use_form && ( ($doptin_check && $doptin_success) || ($set=="save" && $check) ) ) {
	...
	if (DEBUG) $MESSAGE.=tm_debugmessage("parseform show message: subscribe_success");
...
if ($called_via_url) {
	...
	if (DEBUG) $MESSAGE.=tm_debugmessage("called via URL, echo OUTPUT");
	echo $OUTPUT;
} else {
	//wenn included, ausgabe in variable _CONTENT speichern
	if (DEBUG) $MESSAGE.=tm_debugmessage("save output to OUTPUT");
	$_CONTENT.= $OUTPUT;
}
?>