...
/*******************************************************************************/
if (DEBUG) $MESSAGE.=tm_debugmessage("updating adr record");
$code=$ADR[0]['code'];
...
if ($ADR[0]['status'] ==11) {
	if (DEBUG) $MESSAGE.=tm_debugmessage("email was unsubscribed, set status=1, aktiv=1");
	$ADDRESS->setStatus($ADR[0]['id'],1);
	$ADDRESS->setAktiv($ADR[0]['id'],1);
}
$newADRID=$ADR[0]['id'];
if (DEBUG) $MESSAGE.=tm_debugmessage("update adrid is ".$newADRID." code: ".$code);
