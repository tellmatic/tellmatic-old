...
if ($doptin==1 && !empty($code) && !empty($email)) {
	if (DEBUG) $MESSAGE.=tm_debugmessage("double opt in click!");
	$doptin_check=TRUE;
...
if ($doptin_check && !$check_mail[0]) {
	if (DEBUG) $MESSAGE.=tm_debugmessage("invalid email!");
	if ($use_form) {
...
if ($doptin_check && $check_mail[0]) {
	if (DEBUG) $MESSAGE.=tm_debugmessage("validating email");
	//adresse pruefen:
	...
	if (isset($ADR[0]) && check_dbid($ADR[0]['id']) && $ADR[0]['code']==$code) {
		...
		//setstatus adr_id = 3
		if (DEBUG) $MESSAGE.=tm_debugmessage("email found, now setting status to 3: confirmed");
