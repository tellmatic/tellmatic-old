...
if ($set=="save") {
	...
	if ($check) {
		...
		$_MAIN_MESSAGE.="<br>".sprintf(___("Benutzer %s wurde bearbeitet."),"<b>".display($name)."</b>")."";
		if (empty($pass)) {$change_pw=false;$_MAIN_MESSAGE.="<br>".___("Kein Passwort angegeben.");}
		if ($change_pw && strlen($pass)<$minlength_pw) {$change_pw=false;$_MAIN_MESSAGE.="<br>".sprintf(___("Passwort sollte mindestens %s Zeichen haben."),$minlength_pw);}
		if ($change_pw && $pass != $pass2) {$change_pw=false;$_MAIN_MESSAGE.="<br>".___("Bitte geben Sie zweimal das gleiche Passwort an.");}
		if (!$change_pw) {$_MAIN_MESSAGE.="<br>".___("Passwort wurde nicht geändert.");}
		if ($change_pw)	{
			if (!DEMO) {
